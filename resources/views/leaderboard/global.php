<div class="container" style="padding-top: 3.5rem; padding-bottom: 5rem;">
    <div class="section__header" style="text-align: left; margin-bottom: 2.5rem;">
        <span class="eyebrow"><?= e(__('leaderboard.title')) ?></span>
        <h1 style="font-size: 2.5rem;"><?= e(__('leaderboard.title')) ?></h1>
    ...
    <!-- Personal Rank Callout (if logged in) -->
    <?php if ($personal_rank && $pb): ?>
        <div class="card card--glow-cyan" style="margin-bottom: 2rem; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem;">
            <div style="display: flex; align-items: center; gap: 1.25rem;">
                <span class="rank-pill rank-1" style="width: 48px; height: 48px; font-size: 1.3rem;">#<?= (int) $personal_rank ?></span>
                <div>
                    <span class="eyebrow" style="margin: 0; font-size: 0.7rem;">YOUR STANDING</span>
                    <div class="font-display font-bold" style="font-size: 1.5rem; color: #fff;"><?= number_format((int) $pb['score']) ?> PTS</div>
                </div>
            </div>
            <a href="/play" class="btn btn--primary btn--sm">Attempt Record Breaker</a>
        </div>
    <?php endif; ?>

    <!-- Filter Form -->
    <div class="card" style="margin-bottom: 2rem; padding: 1.25rem;">
        <form method="GET" action="/leaderboard" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)) 120px; gap: 1rem; align-items: flex-end;">
            <div>
    ...
    <div class="table-wrap">
        <table class="table" aria-label="Global Verified Leaderboard">
            <thead>
                <tr>
